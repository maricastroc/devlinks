...
import LogoWhite from '/public/assets/images/logo-devlinks-large-white.svg';
...
export default function Shared({ userLinks, user, authUser }: Props) {
  ...
  return (
    <div className="relative flex flex-col items-center min-h-screen pt-6 md:pt-0 md:mt-0 bg-light-gray sm:justify-center">
      <Head title="Shared" />
      {!isOwner && (
      ...
      {!isOwner && (
        <div className="z-50 hidden mt-12 mb-0 md:block">
          <Link href="/">
            <img className="mt-6 scale-75 md:mt-0" src={LogoWhite} alt="" />
          </Link>
        </div>
      )}

      {isOwner && (
        <header className='z-50 w-full h-[78px] md:p-4 md:mb-16'>
          <div className='flex items-center justify-between gap-3 p-4 md:bg-white md:rounded-xl'>
            <SecondaryButton className="p-2 max-w-[10rem]" onClick={() => router.get(route('profile'))}>
